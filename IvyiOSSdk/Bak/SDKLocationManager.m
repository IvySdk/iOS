//
//  SDKLocationManager.m
//  MusicRadio
//
//  Created by 余冰星 on 17/3/7.
//  Copyright © 2017年 IVYMOBILE INTERNATIONAL ENTERPRISE LIMITED. All rights reserved.
//

#import "SDKLocationManager.h"
#import <UIKit/UIKit.h>

@implementation SDKLocationManager
{
@private
    CLLocationManager *manager;
    SDKLocationSuccess successCallBack;
    SDKLocationFailed failedCallBack;
    int repeatCount;
}
+ (SDKLocationManager *) sharedGpsManager {
    static SDKLocationManager *shared = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shared = [[SDKLocationManager alloc] init];
    });
    return shared;
}

- (void)getManager {
    if(!manager) {
        // 打开定位 然后得到数据
        manager = [[CLLocationManager alloc] init];
        manager.delegate = self;
        //控制定位精度,越高耗电量越
        manager.desiredAccuracy = kCLLocationAccuracyBest;
        
        // 兼容iOS8.0版本
        /* Info.plist里面加上2项
         NSLocationAlwaysUsageDescription      Boolean YES
         NSLocationWhenInUseUsageDescription   Boolean YES
         */
        
        // 请求授权 requestWhenInUseAuthorization用在>=iOS8.0上
        // respondsToSelector: 前面manager是否有后面requestWhenInUseAuthorization方法
        // 1. 适配 动态适配
        if ([manager respondsToSelector:@selector(requestWhenInUseAuthorization)]) {
            [manager requestWhenInUseAuthorization];
            [manager requestAlwaysAuthorization];
        }
        // 2. 另外一种适配 systemVersion 有可能是 8.1.1
        float osVersion = [[[UIDevice currentDevice] systemVersion] floatValue];
        if (osVersion >= 8) {
            [manager requestWhenInUseAuthorization];
            [manager requestAlwaysAuthorization];
        }
    }
}

- (void) getSDKLocationWithSuccess:(SDKLocationSuccess)success Failure:(SDKLocationFailed)failure repeatCount:(int)count{
    [self getManager];
    successCallBack = [success copy];
    failedCallBack = [failure copy];
    repeatCount = count;
    // 停止上一次的
    [manager stopUpdatingLocation];
    // 开始新的数据定位
    [manager startUpdatingLocation];
}

+ (void) getSDKLocationWithSuccess:(SDKLocationSuccess)success Failure:(SDKLocationFailed)failure repeatCount:(int)count{
    [[SDKLocationManager sharedGpsManager] getSDKLocationWithSuccess:success Failure:failure repeatCount:count];
}

- (void) stop {
    [manager stopUpdatingLocation];
    manager = nil;
}

- (void) repeat {
    if(--repeatCount < 0)
        [self stop];
}

+ (void) stop {
    [[SDKLocationManager sharedGpsManager] stop];
}

// 每隔一段时间就会调用
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations {
    for (CLLocation *loc in locations) {
        CLLocationCoordinate2D l = loc.coordinate;
        double lat = l.latitude;
        double lnt = l.longitude;
        
        successCallBack ? successCallBack(lat, lnt) : nil;
        [self repeat];
        return;
    }
}

//失败代理方法
- (void)locationManager:(CLLocationManager *)manager didFailWithError:(NSError *)error {
    failedCallBack ? failedCallBack(error) : nil;
    if ([error code] == kCLErrorDenied) {
        DLog(@"[location] 访问被拒绝");
    }
    if ([error code] == kCLErrorLocationUnknown) {
        DLog(@"[location] 无法获取位置信息");
    }
    [self repeat];
}

@end
